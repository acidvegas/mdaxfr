...
## TLDs that allow AXFR
As of the last scan in 2026, the following **15** TLDs allow zone transfers from their official nameservers:
...
Some TLDs that allow AXFR also expose their sub-zones *(e.g. `com.er`, `edu.er`, `org.er`, `ac.mw`, `co.mw`, `com.mw`, etc)*. These sub-zone transfers are handled by the [extras/ozones](./extras/ozones) script, which also covers zones only available through dedicated zone transfer servers like `ch`, `li`, `ee`, `nu`, and `se`.
...
The [addition scripts](./extras/) in this repository provide additional zone files from dedicated zone transfer servers and sub-zones that aren't covered by standard TLD scanning.
...
###### Mirrors for this repository: [SuperNETs](https://git.supernets.org/acidvegas/mdaxfr) • [GitHub](https://github.com/acidvegas/mdaxfr) • [GitLab](https://gitlab.com/acidvegas/mdaxfr) • [Codeberg](https://codeberg.org/acidvegas/mdaxfr)